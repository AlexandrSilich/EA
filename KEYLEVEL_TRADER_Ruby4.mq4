...
int OnInit()
{
   ...
   Print("KEYLEVEL_TRADER_Ruby4 Initialized. v4.00 | FIX: IsMarketDead gate on entry.");
...
// =========================================================
// *** CHAMELEON ADAPTIVE ENTRY ***
// FIX-1: добавлен гейт IsMarketDead ПЕРЕД всеми проверками входа.
//        Если рынок уже dead по критерию выхода — сигнал = 0.
//        Это синхронизирует логику входа и выхода и устраняет
//        сценарий "открыли и сразу получили VOLATILITY_DEAD".
// =========================================================
int GetAdaptiveSignal(string pair, int tf)
{
   ...
   // [FIX-1] Гейт мёртвого рынка — тот же критерий, что и в SmartExitEngine
   // Если IsMarketDead=true, то и войти нельзя — нет смысла открывать позицию,
   // которую придётся закрыть на следующем тике как VOLATILITY_DEAD.
   if(UseVolatilityPulse && IsMarketDead(pair, tf)) return 0;

   // Стандартный ATR-фильтр по одному закрытому бару
...
bool CheckTrigger(string pair, int tf, bool isBuy)
{
   ...
   double high1 = iHigh(pair, tf, 1); double low1  = iLow(pair, tf, 1);
   double high0 = iHigh(pair, tf, 0); double low0  = iLow(pair, tf, 0);
   if(isBuy) {
   ...
   for(int p = 0; p < ArraySize(PairsList); p++) {
      ...
      for(int i = 0; i < ArraySize(TF_Array); i++) {
         ...
         if(UseAdaptiveEntry) {
            ...
            // [FIX-3] Инвалидация кеша: если рынок стал dead на том же баре —
            // сбрасываем кешированный сигнал, чтобы не войти по устаревшим данным.
...
//+------------------------------------------------------------------+
// FIX-2: Если ATR-стоп зажат на MinStopPips (rawStop < MinStopPips),
// это признак слишком тихого рынка -> не открываем ордер.
// Это предотвращает огромные лоты при маленьком стопе (лот ~ 1/stop).
//+------------------------------------------------------------------+
void ExecuteTrade(string pair, int tf, int type, int pIndex, int tfIndex)
{
   ...
   if(UseATRStops) {
      ...
      // [FIX-2] Если сырой ATR-стоп меньше MinStopPips — рынок слишком тихий,
      // лот будет неадекватно крупным. Блокируем вход.
...
double OrderRiskPips(double entry, double tp, double pip)
{
   if(tp <= 0 || InitialTP_RR <= 0 || pip <= 0)
      return (UseATRStops ? MinStopPips : InitialStopPips);
