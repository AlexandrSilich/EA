...
// --- Give-back: защита уже набранной прибыли (закрывает дыру 0..+20) ---
extern double GiveBackActivatePips    = 12.0;  // включается, когда пик прибыли >= этого
extern double GiveBackPercent         = 45.0;  // выход, если отдали > этого % от пика
...
// --- ACE per-position state ---
// Ключ = (OpenTime, Symbol, Type): переживает частичное закрытие (MT4 меняет тикет,
// но сохраняет OpenTime/OpenPrice), поэтому флаги/пик не сбрасываются.
datetime g_aceOpenTime[];
string   g_aceSym[];
int      g_aceType[];
double   g_aceInitRiskPips[];
double   g_acePeakPips[];
bool     g_acePartialDone[];

//+------------------------------------------------------------------+
int OnInit()
{
   ArrayInitialize(LastTradeTime, 0);
   ArrayResize(g_aceOpenTime, 0);
   ArrayResize(g_aceSym, 0);
   ArrayResize(g_aceType, 0);
   ArrayResize(g_aceInitRiskPips, 0);
   ArrayResize(g_acePeakPips, 0);
   ArrayResize(g_acePartialDone, 0);
...
// =========================================================
// *** ADAPTIVE CONVICTION EXIT (ACE) ENGINE ***
// FIX: все детекторы теперь считаются по ТФ ОРДЕРА (GetOrderTF), не по хардкоду M15.
// Добавлено: единый балл убеждённости S, защита пика прибыли (give-back),
// частичная фиксация на +1R с переводом остатка в безубыток.
// =========================================================
void SmartExitEngine()
{
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      ...
      // регистрируем/находим состояние позиции и обновляем пик прибыли
      int idx = AceRegister();
      if(currentPnlPips > g_acePeakPips[idx]) g_acePeakPips[idx] = currentPnlPips;
      double peak = g_acePeakPips[idx];
      double R    = g_aceInitRiskPips[idx];
      ...
      // --- Частичная фиксация на +1R: режем 50%, остаток -> безубыток ---
      if(UsePartialClose && !g_acePartialDone[idx] && R > 0 &&
         currentPnlPips >= PartialAtR * R) {
         DoPartialAndBE(ticket);
         g_acePartialDone[idx] = true;
         continue;   // остатком займёмся на следующем тике
      }

      double S = ConvictionScore(pair, tf, type);

      if(currentPnlPips > 0) {
         // --- Give-back: отдали слишком много от пика -> банкуем ---
         if(peak >= GiveBackActivatePips &&
            currentPnlPips <= peak * (1.0 - GiveBackPercent / 100.0)) {
            CloseOrderByTicket(ticket, "GIVEBACK_peak" + DoubleToString(peak,1));
            continue;
         }
         // --- Жёсткий разворот убеждённости против ПОДТВЕРЖДЁННОЙ прибыли ---
         // gate by peak: фейд стартует с S<0, поэтому банкуем только когда прибыль
         // реально состоялась (пик >= порога), иначе зарежем победителя слишком рано.
         if(peak >= GiveBackActivatePips && S <= -ConvictionExitProfit) {
         ...
         if(currentPnlPips <= -MomDeathMinPipsLoss && SARBetrayalDetected(pair, tf, type, pip)) {
            CloseOrderByTicket(ticket, "SAR_BETRAYAL");
            continue;
         }
         // --- Убеждённость уверенно против нас ---
         if(currentPnlPips <= -MomDeathMinPipsLoss && S <= -ConvictionExitLoss) {
...
// =========================================================
// ACE: реестр состояния позиций (ключ = OpenTime+Symbol+Type)
// =========================================================
int AceRegister()
{
   ...
   for(int i = 0; i < ArraySize(g_aceOpenTime); i++)
      if(g_aceOpenTime[i] == ot && g_aceSym[i] == s && g_aceType[i] == t) return i;

   int n = ArraySize(g_aceOpenTime);
   ArrayResize(g_aceOpenTime,     n + 1);
   ArrayResize(g_aceSym,          n + 1);
   ArrayResize(g_aceType,         n + 1);
   ArrayResize(g_aceInitRiskPips, n + 1);
   ArrayResize(g_acePeakPips,     n + 1);
   ArrayResize(g_acePartialDone,  n + 1);

   double pip = PipOf(s);
   g_aceOpenTime[n]     = ot;
   g_aceSym[n]          = s;
   g_aceType[n]         = t;
   g_aceInitRiskPips[n] = (OrderStopLoss() > 0) ? MathAbs(OrderOpenPrice() - OrderStopLoss()) / pip : 60.0;
   g_acePeakPips[n]     = 0.0;
   g_acePartialDone[n]  = false;
   return n;
}

void AcePrune()
{
   for(int i = ArraySize(g_aceOpenTime) - 1; i >= 0; i--) {
      bool found = false;
      for(int j = OrdersTotal() - 1; j >= 0; j--) {
         if(!OrderSelect(j, SELECT_BY_POS, MODE_TRADES)) continue;
         if(OrderMagicNumber() != MagicNumber) continue;
         if(OrderOpenTime() == g_aceOpenTime[i] && OrderSymbol() == g_aceSym[i] &&
            OrderType() == g_aceType[i]) { found = true; break; }
      }
      if(found) continue;

      int last = ArraySize(g_aceOpenTime) - 1;
      g_aceOpenTime[i]     = g_aceOpenTime[last];
      g_aceSym[i]          = g_aceSym[last];
      g_aceType[i]         = g_aceType[last];
      g_aceInitRiskPips[i] = g_aceInitRiskPips[last];
      g_acePeakPips[i]     = g_acePeakPips[last];
      g_acePartialDone[i]  = g_acePartialDone[last];
      ArrayResize(g_aceOpenTime,     last);
      ArrayResize(g_aceSym,          last);
      ArrayResize(g_aceType,         last);
      ArrayResize(g_aceInitRiskPips, last);
      ArrayResize(g_acePeakPips,     last);
      ArrayResize(g_acePartialDone,  last);
   }
}
