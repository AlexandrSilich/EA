...
extern double ERRangeLevel       = 0.20;    // ER <= -> боковой режим (между = пропуск)
...
extern bool   UseHTFBias         = true;    // согласование со старшим ТФ

// Боковой режим (fade)
extern int    RangeLookback      = 20;      // период канала Donchian
extern int    RangeRSIPeriod     = 14;
extern double RangeRSIOver       = 70.0;    // перекупленность
extern double RangeRSIUnder      = 30.0;    // перепроданность
extern double RangeEdgePct       = 0.15;    // "край" канала = эта доля ширины
...
extern double VolatilityDeadZoneATR   = 0.0006;
...
void SmartExitEngine()
{
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
      ...
      // --- Частичная фиксация на +1R: фаза читается из SL ---
